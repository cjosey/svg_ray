...
"""
type Tally
    data::Array{Float32, 3} # 3D array of dimension [x y wavelength]
    x::Int                  # Size in x
    y::Int                  # Size in y
    nw::Int                 # Size in wl
    scaling_factor::Number  # Ratio of actual dimensions to tally dimensions.  If > 1, increases tally size.
    n_low::Number           # Low wavelength
    n_high::Number          # High wavelength
end

"""
A constructor for the tally with some defaults
"""
function Tally(x::Number, y::Number, scaling_factor=1, nw=80)
    x = round(Int, x * scaling_factor)
    y = round(Int, y * scaling_factor)

    data = zeros(Float32, x ,y ,nw)

    n_low = 380
    n_high = 780

    Tally(data, x, y, nw, scaling_factor, n_low, n_high)
end
...
function +(a::Tally, b::Tally)
    # Generate merged
    temp = a.data + b.data
    Tally(temp, a.x, a.y, a.nw, a.scaling_factor, a.n_low, a.n_high)
end
...
function tally_path(t, path, wl, scale)
    wl_ind = round(Int, (wl - t.n_low) / (t.n_high - t.n_low) * (t.nw-1)) + 1

    for i = 1:size(path)[1] - 1
        p1 = path[i,:]
        p2 = path[i+1,:]

        tally_line_hq(t, p1, p2, wl_ind, scale[i])
    end
end
...
"""
function tally_line_hq(t, p1, p2, wl_ind, scale)
...
"""
Converts a spectrum tally to an HDR XYZ tally
"""
function tally_to_xyz(t)
    x = t.x
    y = t.y
    ...
    t2 = Tally(zeros(Float32, x, y, 3), x, y, 3, t.scaling_factor, 0, 1)
    ...
    for i = 1:x
        for j = 1:y
            t2.data[i,j,:] = cie_color_match * reshape(t.data[i,j,:],80)
        end
    end
    t2
end
